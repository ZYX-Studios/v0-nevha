...
import { ArrowLeft, Calendar, CheckCircle, Clock, AlertCircle, Tag, Mail, RefreshCw, MapPin, LogOut } from "lucide-react"
...
export default function DeptIssueDetailPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    try {
      ...
      const [r1, r2] = await Promise.all([
        fetch(`/api/dept/issues/${issueId}`, { cache: "no-store" }),
        fetch(`/api/dept/issues/${issueId}/updates`, { cache: "no-store" }),
      ])
  ...
  const reloadUpdates = async () => {
    if (!issueId) return
    try {
      const r = await fetch(`/api/dept/issues/${issueId}/updates`, { cache: "no-store" })
  ...
  function ensureDisplayName(): string | null {
    const name = (displayName || localStorage.getItem("dept_display_name") || "").trim()
    if (!name) {
      toast.error("Please enter your name on the login page")
  ...
  async function addUpdate() {
    ...
    setSaving(true)
    try {
      const payload: any = { author_label: author }
      if (notes.trim()) payload.notes = notes.trim()
      const res = await fetch(`/api/dept/issues/${item.id}/updates`, {
        method: "POST",
      ...
      setUpdates((prev) => [{ id: "optimistic", status: "in_progress", notes: notes.trim() || null, authorLabel: author, createdAt: new Date().toISOString() }, ...prev])
      setNotes("")
      await load()
    } catch (e) {
  ...
  async function toggleHold() {
    ...
    try {
      ...
      const payload: any = { status: targetStatus, author_label: author }
      if (notes.trim()) payload.notes = notes.trim()
      const res = await fetch(`/api/dept/issues/${item.id}/updates`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setUpdates((prev) => [{ id: "optimistic", status: targetStatus, notes: notes.trim() || null, authorLabel: author, createdAt: new Date().toISOString() }, ...prev])
      setNotes("")
      await load()
    } catch (e) {
  ...
  const handlePriorityChange = async (newPriority: string) => {
    if (!item) return
    try {
      const res = await fetch(`/api/dept/issues/${item.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="sm" onClick={() => router.push("/dept/issues")} className="flex items-center gap-2">
